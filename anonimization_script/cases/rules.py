...
class InheritanceRule(RuleAbstract):
    ...
    """
    def __init__(self, row: dict, route: str, mapper: SimpleMapper, value: Any):
        self._row = row
        self._mapper = mapper
        self._route = route
        self._value = value
...
class TemplateRule(RuleAbstract):
    ...
    def __init__(self, row: dict, mapper: SimpleMapper, value: Any, template: str):
        self._row = row
        self._mapper = mapper
        self._value = value
    ...
    def __call__(self):
        ...
        for i, group in enumerate(groups):
            ...
            _rule_class: RuleAbstract = RuleFactory.get_rule(_regexes[i][0])
            _rule = _rule_class.resolve(_data_for_resolve)
            _result_from_rule = _rule()
            
            new_value = self._value.replace(group, _result_from_rule)
            
            """
            if _regexes[i][0] == "inheritance":
                ...
                result = tr()
                new_value = self._value.replace(group, result)

            elif _regexes[i][0] == "devices_name_fake":
                tr = DevicesNameFake()
                result = tr()
                
                new_value = self._value.replace(group, result)
                
            elif _regexes[i][0] == "ipv4_private_random":
                tr = Ipv4PrivateRule()
                result = tr()
                new_value = self._value.replace(group, result)
            """
